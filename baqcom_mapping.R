...
option_list <- list(
    ...
    make_option(c('-m', '--genome'), type = 'character', default = 'index_STAR',
                help = 'Folder that contains gtf file [default %default]',
                dest = 'genomeDir'),
    make_option(c("-t", "--mappingTargets"), type="character", default="genome_target.fa",
                help="Path to a fasta file, to run mapping against [default %default]",
                dest="mappingTarget"),
    make_option(c("-g", "--gtfTargets"), type="character", default="gtf_file_target.gtf",
                help="Path to a gtf file, to run mapping against [default %default]",
                dest="gtfTarget"),
...
'Version: 0.0.1', 'E-mail: hanielcedraz@gmail.com', sep = "\n", collapse = '\n')))
